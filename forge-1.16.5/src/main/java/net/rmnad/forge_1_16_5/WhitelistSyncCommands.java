...
public class WhitelistSyncCommands {
    ...
    private static void syncWhitelist(CommandContext<CommandSource> context) throws CommandSyntaxException {
    ...
    private static void syncOps(CommandContext<CommandSource> context) throws CommandSyntaxException {
    ...
    private static void syncBannedPlayers(CommandContext<CommandSource> context) throws CommandSyntaxException {
        if (WhitelistSyncCore.whitelistService.pullDatabaseBannedPlayersToLocal()) {
            context.getSource().sendSuccess(new StringTextComponent("Local banned players up to date with database."), false);
        } else {
            throw SYNC_DB_ERROR.create();
        }
    }

    private static void syncBannedIps(CommandContext<CommandSource> context) throws CommandSyntaxException {
        if (WhitelistSyncCore.whitelistService.pullDatabaseBannedIpsToLocal()) {
            context.getSource().sendSuccess(new StringTextComponent("Local banned ips up to date with database."), false);
        } else {
            throw SYNC_DB_ERROR.create();
        }
    }
    ...
    // Lambdas for each sync method to reduce code duplication
    private static void pushWhitelist(CommandContext<CommandSource> context) throws CommandSyntaxException {
    ...
    private static void pushOps(CommandContext<CommandSource> context) throws CommandSyntaxException {
    ...
    private static void pushBannedPlayers(CommandContext<CommandSource> context) throws CommandSyntaxException {
        if (WhitelistSyncCore.whitelistService.pushLocalBannedPlayersToDatabase()) {
            context.getSource().sendSuccess(new StringTextComponent("Pushed local banned players to database."), false);
        } else {
            throw PUSH_DB_ERROR.create();
        }
    }

    private static void pushBannedIps(CommandContext<CommandSource> context) throws CommandSyntaxException {
        if (WhitelistSyncCore.whitelistService.pushLocalBannedIpsToDatabase()) {
            context.getSource().sendSuccess(new StringTextComponent("Pushed local banned ips to database."), false);
        } else {
            throw PUSH_DB_ERROR.create();
        }
    }
    ...
    static ArgumentBuilder<CommandSource, ?> registerSync() {
        LiteralArgumentBuilder<CommandSource> syncBaseCommand = Commands.literal("sync")
                .executes(context -> {
        ...
        syncBaseCommand = syncBaseCommand.then(Commands.literal("op").executes(context -> {
            syncOps(context);
            return 0;
        }));
    ...
    static ArgumentBuilder<CommandSource, ?> registerPush() {
        LiteralArgumentBuilder<CommandSource> pushBaseCommand = Commands.literal("push")
                .executes(context -> {
        ...
        pushBaseCommand = pushBaseCommand.then(Commands.literal("op").executes(context -> {
            pushOps(context);
            return 0;
        }));
